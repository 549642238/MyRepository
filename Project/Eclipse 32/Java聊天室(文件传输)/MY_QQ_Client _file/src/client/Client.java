...
class Client_GUI extends JApplet{
	...
	private PrintWriter fwf;
	protected static JFrame f = null;
	private String no = null;
	private Icon picture = new ImageIcon("picture & sound\\1.jpg");
	private String fileDownLoadPath = "file\\";
	private String sendFilePath = new String("");
	protected JTextArea ta1 = new JTextArea(20,20);
	private JTextArea ta2 = new JTextArea(2,13);
	private JTextArea ta3 = new JTextArea(2,22);
	...
	private JButton b4 = new JButton("File");		//是否接受文件
	private JButton b5 = new JButton("Recv");
	private JLabel l = new JLabel("Destination"); 
	private JTextField t =new JTextField(10);
	private final String ipv4 = "localhost";		//server ip
	private String latest = "";						//最近收到文件名称
	...
	public void init(){
		...
		b4.setToolTipText("Choose the path of File");
		b4.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
		        ...
		        if(file == null){
		        	...
		        	sendFilePath = jfc.getSelectedFile().getAbsolutePath();
		        	b4.setText(jfc.getSelectedFile().getName());
		        }
			}
		});
		c.add(b4);
		b5.setToolTipText("Receive the file?");
		b5.setBackground(Color.PINK);
		b5.setBorder(new LineBorder(Color.RED));
		b5.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				try {
					String line;
					File tempfile = new File("shaxiang\\"+latest);
					fwf = new PrintWriter(new FileOutputStream(fileDownLoadPath+latest));
					BufferedReader rd = new BufferedReader(new FileReader("shaxiang\\" + latest));
					...
					System.out.println("Client error10");
					e2.printStackTrace();
				}
			}
		});
		...
		b1.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){			/*	
																...
																*/
				if(ta2.getText().equals("") && sendFilePath.equals("")){
					Sound.play();
					ta1.append("System:You send Nothing!\n");
					...
					try{
						...
						writer.write(no+"\n");
						writer.write(t1.getText()+"\n");
						File fi = new File(sendFilePath);
						String recv = t1.getText();
						...
						writer.write(fi.getName()+"\n");
						writer.write("[发送给："+recv+"]"+"\n");
						BufferedReader fb = new BufferedReader(new InputStreamReader(new FileInputStream(sendFilePath)));
						String temp = "";
						while((temp=fb.readLine())!=null){
							writer.write(temp+"\n");
						}
						fb.close();
						writer.write("EOF\n");
			...
			while(!stop){
				...
				try{
					...
					if(s0.equals("AddText")){				//更新房间信息的步骤一，也可以用来就收来自其他client的消息
						...
						ta3.setText(s1+"\n"+s2);
					}else if(s0.equals("File")){
						Sound.play();
						try{
							String temp = "";
							ta1.append(br.readLine()+":"+(temp=br.readLine())+" "+br.readLine()+"\n");
							latest = temp;
							PrintWriter fw = new PrintWriter(new FileOutputStream("shaxiang\\"+temp));
							while(!(temp = br.readLine()).equals("EOF")){
								fw.write(temp+"\n");
							}
							fw.flush();
							fw.close();
...
class Sound{
	public static void play(){
		File f1 = new File("picture & sound\\a.wav"); 
		Player player=null;
		try{
