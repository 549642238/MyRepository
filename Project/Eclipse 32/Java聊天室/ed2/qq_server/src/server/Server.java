...
public class Server{
	...
	public static synchronized boolean addClient(){													//当一个Client在执行时，如若又来一个Client发现server没监听会连接失败，所以需要原子操作
		boolean b = true;
		try{
			ServerSocket server = new ServerSocket(8189);
			Socket client = server.accept();														//服务器开启后会阻塞在这里
			...
			if(!b){
				...
				System.out.println(temp+" link success");
				br.readLine();
			}
			br.close();
			writer.close();
	...
	public Server(int port){
		try{
			...
			while(true){
				try{
					synchronized (this){					//同步块保证先进来的能先执行Welcome()，使得房间信息和最新登录者同步
						if(addClient() == false){			
							continue label;					/*
...
class ThreadServerHandler extends Thread implements Runnable{
	...
	public void write(String s0,String s1,String s2){
		try{
			writer.write(s0+"\n");			//当两客户端同时发给同一个Client，会调用同一个Thread中write()两次，不需要使用synchronized
			writer.write(s1+"\n");
			writer.write(s2+"eof\n");
			writer.flush();
		}catch(Exception ee){
         		...
         		if(s3.equals("Exit_MyQQ")){
         			...
         			for(int i=0;i<Server.toList().size();i++){
         				if(((Client)(Server.toList().get(i))).toNo().equals(s1)){
         					Server.toList().remove(i);			//更新房间动态前一定要将他remove掉，不解释，不然出错(记录)
         					break;
         				}
         			}
         			for(int i=0;i<Server.toL().size();i++){
