        				...
        				if(passwd.equals(rs.getString("passwd"))){
        					try{
								...
								if(s.equals("Yes")){								/*
																						...
																						的弹框提示"账户已在线"
																					*/
									ServerSocket ss = new ServerSocket(8989);		//目的：阻塞，等待Server完成相应步骤，如果去掉不太安全
									Socket cc =ss.accept();
									BufferedReader bbr = new BufferedReader(new InputStreamReader(cc.getInputStream()));
									Writer wwriter = new OutputStreamWriter(cc.getOutputStream());
									if(bbr.readLine().equals("Run")){
										wwriter.write("end\n");
										wwriter.flush();
										cl.run();
										wwriter.write("exit\n");
