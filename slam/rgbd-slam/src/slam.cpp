...
CHECK_RESULT checkKeyframes(FRAME& frame1,FRAME& frame2,g2o::SparseOptimizer& opti,bool is_loops=false){
	static ParameterReader pr;
	static double good_match_threshold = atof((pr.getData("good_match_threshold")).c_str());
	static int  min_good_match = atoi((pr.getData("min_good_match")).c_str());
	static int min_inliers = atoi((pr.getData("min_inliers")).c_str());
	static double max_norm = atof((pr.getData("max_norm")).c_str());
	static double keyframe_threshold = atof((pr.getData("keyframe_threshold")).c_str());
	static double max_norm_lp = atof((pr.getData("max_norm_lp")).c_str());
	static CAMERA_INTRINSIC_PARAMETERS camera = getDefaultCamera();
	static g2o::RobustKernel* robustKernel = g2o::RobustKernelFactory::instance()->construct("Cauchy");
	// 比较frame1 和 frame2
	RESULT_OF_PNP result = estimateMotion(frame1,frame2,camera,good_match_threshold,min_good_match);
...
int main(int argc,char** argv){
	...
	double keyframe_threshold = atof((pr->getData("keyframe_threshold")).c_str());
	bool check_loop_closure = strcmp((pr->getData("check_loop_closure")).c_str(),"yes") == 0;
	...
	//存储
	pcl::io::savePCDFile("../pic/result.pcd",*tmp);
