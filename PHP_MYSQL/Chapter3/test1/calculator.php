<?php 
	# Script 3.4 - calculator.php
	function create_gallon_price($value){
	...
	function calculate_price($mile,$mpg,$ppg){
		$gallon = $mile/$mpg;
		$dollar = $gallon*$ppg;
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (isset($_POST['distance'], $_POST['gallon_price'], $_POST['efficiency']) && is_numeric($_POST['distance']) && is_numeric($_POST['gallon_price']) && is_numeric($_POST['efficiency']) ) {
			...
			$dollars = calculate_price($_POST['distance'], $_POST['efficiency'], $_POST['gallon_price']);
			$hours = $_POST['distance']/65;
			// Print the results:
			echo '<h1>Total Estimated Cost</h1>
			<p>The total cost of driving ' . $_POST['distance'] . ' miles, averaging ' . $_POST['efficiency'] . ' miles per gallon, and paying an average of $' . $_POST['gallon_price'] . ' per gallon, is $' . $dollars . '. If you drive at an average of 65 miles per hour, the trip will
...
<form action="calculator.php" method="post">
	...
	<p>Fuel Efficiency: <select name="efficiency">
		...
		<option value="50"
			<?php
				if (isset($_POST['efficiency']) && ($_POST['efficiency'] == '50')){
					echo ' selected="selected"';
				}
			?>
		>Outstanding</option>
