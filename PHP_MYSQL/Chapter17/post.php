	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
		...
		if ($subject && $body) { // OK!
			...
			if (!$tid) { // Create a new thread.
				$q = "INSERT INTO threads (lang_id, user_id, subject) VALUES ({$_SESSION['lid']},{$_SESSION['user_id']}, '" . mysql_real_escape_string($subject) . "')";
				$r = mysql_query($q);
				if (mysql_affected_rows($dbc) == 1) {
					$tid = mysql_insert_id($dbc);
				} else {
					echo '<p>Your post could not be handled due to a system error.</p>';
				}
			} // No $tid. 11
			if ($tid) { // Add this to the replies table:
				$q = "INSERT INTO posts (thread_id, user_id, message, posted_on) VALUES ($tid,{$_SESSION['user_id']}, '" . mysql_real_escape_string($body) . "', UTC_TIMESTAMP())";
				$r = mysql_query($q);
				if (mysql_affected_rows($dbc) == 1) {
